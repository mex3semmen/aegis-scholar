...
Phase 110.0 is a backend-only OpenAlex Metadata Execution Slice. It composes the Scientific Metadata Provider Request Preview, stays consent-gated and disabled by default, and only opens the OpenAlex-only execution gate; it does not add Crossref, PubMed, ERIC, APA PsycNet, or PsycINFO execution, writes, citations, Evidence Packs, or Literature Reviews.
