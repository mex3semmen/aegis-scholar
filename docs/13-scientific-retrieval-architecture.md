...
Phase 109.0 is a docs-only Scientific Retrieval Contract Map / GUI Integration Readiness pass. It stabilizes the GUI/backend contract understanding before the first execution slice, documents refactor seams and the OpenAlex-only execution cutover, and does not change runtime or execution behavior.
Phase 110.0 is a backend-only OpenAlex Metadata Execution Slice. It is the first execution cutover from preview contract to OpenAlex-only metadata execution, keeps provider execution narrow and consent-gated, and returns normalized in-memory metadata records only. It does not expand into broad retrieval, citation generation, Evidence Pack creation, Literature Review creation, or answer generation. Crossref, PubMed, and ERIC remain later providers, while APA PsycNet and PsycINFO remain an institutional/manual boundary only.
Phase 110.1 is a backend-only OpenAlex Metadata Execution Slice Guard Hardening pass. It keeps the first execution slice narrow, gated, offline-tested, in-memory, and non-writing, and it does not broaden provider support or retrieval / answer behavior.
Phase 111.0 is a backend-only OpenAlex Metadata Result Normalization Contract. It refines the OpenAlex execution output contract for GUI and later Evidence Pack phases, keeps the output in-memory, redacted, and non-writing, and does not broaden retrieval or answer generation.
Phase 112.0 is a backend-only OpenAlex Metadata Cache/Write Gate Preview. It composes the OpenAlex Metadata Execution Slice and only plans future cache scope, retention, deduplication, record-write, and audit boundaries. It stays preview-only and does not add cache writes, record writes, audit writes, retrieval execution, Evidence Pack creation, Literature Review creation, answer generation, or broader provider behavior.
Phase 112.1 is backend-only OpenAlex Metadata Cache/Write Gate Preview Guard Hardening. It test-documents the command surface, no-op flags, fake-transport-only execution composition, policy normalization, path safety, dedup/readiness planning, redaction, and no-side-effect invariants. It stays preview-only, in-memory, redacted, and non-writing and does not add persistence or broaden retrieval/provider behavior.
Phase 113.0 is a docs-only OpenAlex Metadata GUI Integration Readiness Contract. It records the safe GUI integration boundary for the existing OpenAlex metadata commands before frontend wiring, and it does not add UI, provider execution, writes, retrieval execution, Evidence Pack creation, Literature Review creation, answer generation, runtime/model behavior, or provider expansion.
Phase 114.0 is a frontend read-only OpenAlex Metadata GUI Scaffold. It surfaces the existing provider-request preview in a developer / advanced panel, keeps execution and cache/write as diagnostics only, and does not add a write button, provider execution, writes, Evidence Pack creation, Literature Review creation, answer generation, runtime/model behavior, or provider expansion.
Phase 114.1 is frontend/docs guard hardening for the read-only OpenAlex panel. It keeps only provider request preview wired, keeps execution and cache/write unwired, and does not add backend, dependency, or product behavior.
...
Phase 120.0 adds frontend-first source workflow action hints. It tightens the manual register -> extract -> chunk -> retrieval preview -> Evidence Pack preview guidance after source registration or selection, and it stays guidance-only. It does not add automatic orchestration, drag-and-drop import, OCR, broad PDF ingestion, web requests, scraping, downloads, connectors, source import behavior, model loading, runtime inference, LLM calls, answer generation, Evidence Pack creation, artifact writes, persistence, registry status changes, or audit writes.
